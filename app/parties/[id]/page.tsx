...
export async function generateMetadata({ params }: PageProps) {
  ...
  return { title: `${party?.name ?? 'Party'} — Elezzjoni.mt` }
}
...
export default async function PartyPage({ params }: PageProps) {
  ...
  const districtIds = [...new Set(candidates?.flatMap((c) => c.districts ?? []) ?? [])].sort(
    (a, b) => a - b
  )
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
        ...
        <span>/</span>
        <Link href="/parties" className="hover:text-foreground transition-colors">Parties</Link>
        <span>/</span>
      ...
      {/* Party header */}
      <div
        className="rounded-xl p-8 mb-10 text-white relative overflow-hidden"
      ...
      >
        ...
        <div className="relative flex items-center gap-5">
          {party.logo_url ? (
            ...
            <div className="h-18 w-18 rounded-lg bg-white/20 flex items-center justify-center text-2xl font-bold px-4 py-3">
              {party.abbreviation}
            </div>
          )}
          <div>
      ...
      {candidates && candidates.length > 0 ? (
        ...
        <p className="text-center text-muted-foreground py-16">No candidates found for this party.</p>
      )}
    </div>
  )
}
