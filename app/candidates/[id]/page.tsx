...
import { MapPin, Globe, Quote, Sparkles, Clock, Share2 } from 'lucide-react'
...
export async function generateMetadata({ params }: PageProps) {
  ...
  return { title: `${data?.full_name ?? s.candidates.fallback} — Elezzjoni.mt` }
}
...
export default async function CandidatePage({ params }: PageProps) {
  ...
  const lastUpdated = new Date(candidate.last_updated).toLocaleDateString('mt-MT', {
        ...
        <div className="p-6 sm:p-8">
          <div className="flex flex-col sm:flex-row gap-6 items-start">
            <div className="shrink-0">
              {candidate.photo_url ? (
                <Image src={candidate.photo_url} alt={candidate.full_name} width={120} height={120}
                  className="rounded-2xl object-cover h-28 w-28 sm:h-32 sm:w-32 ring-4 ring-white shadow" />
              ) : (
                <div className="h-28 w-28 sm:h-32 sm:w-32 rounded-2xl flex items-center justify-center text-4xl font-bold text-white shadow ring-4 ring-white"
              ...
              <div className="flex flex-wrap gap-2 mb-2">
                {party && (
                  <Link href={`/parties/${party.id}`}>
                    <Badge className="text-xs font-bold border-0 text-white hover:opacity-90 transition-opacity"
                ...
                {candidate.incumbent && (
                  <Badge variant="secondary" className="text-xs">{s.candidateProfile.incumbent}</Badge>
                )}
              </div>
        ...
        {candidate.personal_stances && candidate.personal_stances.length > 0 && (
          <Card className="p-6 border-border/60 shadow-sm">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-semibold text-base">{s.candidateProfile.stancesTitle}</h2>
              <Badge variant="secondary" className="text-[10px] ml-auto">{s.candidateProfile.stancesBadge}</Badge>
            </div>
        ...
        {candidate.key_quotes && candidate.key_quotes.length > 0 && (
          <Card className="p-6 border-border/60 shadow-sm">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-semibold text-base">{s.candidateProfile.quotesTitle}</h2>
              <Badge variant="secondary" className="text-[10px] ml-auto">{s.candidateProfile.quotesBadge}</Badge>
            </div>
            <div className="space-y-4">
              {candidate.key_quotes.map((quote: string, i: number) => (
                <blockquote key={i} className="pl-4 border-l-2 text-sm italic text-muted-foreground leading-relaxed"
                  style={{ borderColor: party?.color_hex ?? '#E5E7EB' }}>
                  "{quote}"
                </blockquote>
              ))}
            </div>
